package io.github.martinhaeusler.lsm4k.io.format.datablock

import io.github.martinhaeusler.lsm4k.io.format.BlockMetaData
...
import io.github.martinhaeusler.lsm4k.util.TreeMapUtils
...
import io.github.martinhaeusler.lsm4k.util.cursor.NavigableMapCursor
...
class DataBlock(
    ...
    companion object {
        ...
        fun loadBlock(
            ...
            val magicBytes = buffer.takeBytes(LSMFileFormat.BLOCK_MAGIC_BYTES.size)
            if (magicBytes != LSMFileFormat.BLOCK_MAGIC_BYTES) {
                throw IllegalArgumentException(
                    "Cannot read block from input: the magic bytes do not match!" +
                        " Expected ${LSMFileFormat.BLOCK_MAGIC_BYTES.hex()}, found ${magicBytes}!"
                )
            }
